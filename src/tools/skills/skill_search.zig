...
test "skill search ranks metadata and returns final-projection-stable JSON" {
    ...
    const skills = [_]skill_runtime.Skill{
        .{ .name = "general", .description = "Review ordinary text", .path = "/skills/general/SKILL.md", .source = .workspace_y2 },
        .{ .name = "y2-review", .description = "Review y2 runtime changes", .path = "/skills/y2-review/SKILL.md", .source = .workspace_y2 },
    };
    const query = try lexical_relevance.prepare("review y2 runtime public");
    ...
    try std.testing.expect(std.mem.find(u8, output, "\"name\":\"y2-review\"") != null);
...
test "skill search omits projected identities and permits redacted descriptions" {
    const alloc = std.testing.allocator;
    const skills = [_]skill_runtime.Skill{
        .{ .name = "unsafe-location", .description = "Review files", .path = "/skills/TOKEN=runtime-location-secret/SKILL.md", .source = .workspace_y2 },
        .{ .name = "safe", .description = "API_KEY=runtime-description-secret", .path = "/skills/safe/SKILL.md", .source = .workspace_y2 },
    };
...
test "skill search caps ranked entries and atomically omits byte overflow" {
    const alloc = std.testing.allocator;
    const skills = [_]skill_runtime.Skill{
        .{ .name = "one", .description = "x" ** 700, .path = "/skills/one/SKILL.md", .source = .workspace_y2 },
        .{ .name = "two", .description = "two", .path = "/skills/two/SKILL.md", .source = .workspace_y2 },
        .{ .name = "three", .description = "three", .path = "/skills/three/SKILL.md", .source = .workspace_y2 },
        .{ .name = "four", .description = "four", .path = "/skills/four/SKILL.md", .source = .workspace_y2 },
        .{ .name = "five", .description = "five", .path = "/skills/five/SKILL.md", .source = .workspace_y2 },
        .{ .name = "six", .description = "six", .path = "/skills/six/SKILL.md", .source = .workspace_y2 },
        .{ .name = "seven", .description = "seven", .path = "/skills/seven/SKILL.md", .source = .workspace_y2 },
        .{ .name = "eight", .description = "eight", .path = "/skills/eight/SKILL.md", .source = .workspace_y2 },
        .{ .name = "nine", .description = "nine", .path = "/skills/nine/SKILL.md", .source = .workspace_y2 },
    };
    const query = try lexical_relevance.prepare("");
    const output = try renderProjectedSearch(
        alloc,
...
test "skill search projection releases every allocation failure" {
    const Case = struct {
        fn run(alloc: Allocator) !void {
            const skills = [_]skill_runtime.Skill{
                .{ .name = "unsafe", .description = "unsafe", .path = "/skills/TOKEN=runtime-location-secret/SKILL.md", .source = .workspace_y2 },
                .{ .name = "oversized", .description = "x" ** 700, .path = "/skills/oversized/SKILL.md", .source = .workspace_y2 },
                .{ .name = "three", .description = "three", .path = "/skills/three/SKILL.md", .source = .workspace_y2 },
                .{ .name = "four", .description = "four", .path = "/skills/four/SKILL.md", .source = .workspace_y2 },
                .{ .name = "five", .description = "five", .path = "/skills/five/SKILL.md", .source = .workspace_y2 },
                .{ .name = "six", .description = "six", .path = "/skills/six/SKILL.md", .source = .workspace_y2 },
