...
pub const StreamState = struct {
    ...
    /// When y2 started waiting on user input (approval or question); 0 means
...
pub const ToolCall = struct {
    ...
    provenance: ToolExecutionProvenance = .y2_local,
};
...
pub const TerminalFailurePresentation = enum {
    ...
    pub fn detail(self: TerminalFailurePresentation) []const u8 {
        return switch (self) {
            ...
            .authority_retired => "saved terminal authority is from an older y2 version; start a new terminal",
...
test "HistoryTurn helpers duplicate and free owned turns" {
    ...
    const background_original: HistoryTurn = .{ .background_command = .{
        ...
        .log_path = try alloc.dupe(u8, "/tmp/y2.log"),
        .expect_url = true,
    ...
    try std.testing.expectEqualStrings("/tmp/y2.log", background_copy.background_command.log_path);
    ...
    const interrupted_original: HistoryTurn = .{ .interrupted = .{
        ...
        .cancelled_command = .{
            .output_replay = .{ .available = .{
                .handle = try alloc.dupe(u8, "y2-command-replay.bin"),
                .framed_bytes = 42,
            } },
            .command_artifact_handle = try alloc.dupe(u8, "y2-command.log"),
        },
