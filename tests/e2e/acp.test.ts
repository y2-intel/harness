...
describe("acp: model-independent", () => {
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot("y2-acp-markdown-source-");
      const markdown = [
        "# Heading\n\n- **bold** item\n\n",
        "| A | B |\n| - | - |\n| 1 | 2 |\n\n",
        "```zig\nconst x = 1;\n```\n",
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot("y2-acp-project-mcp-");
        ...
        JSON.stringify({
          mcpServers: {
            fixture: {
              ...
              env: {
                Y2_MCP_RESULT_TEXT: "${ACP_PROJECT_RESULT:-ACP_PROJECT_MCP_RESULT}",
                Y2_MCP_PID_PATH: "${ACP_PROJECT_PID}",
                Y2_MCP_WIRE_LOG: "${ACP_PROJECT_WIRE}",
              },
            },
          },
        }),
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot("y2-acp-project-mcp-collision-");
        ...
        JSON.stringify({
          mcpServers: {
            fixture: {
              command: process.execPath,
              args: [MCP_STDIO_FIXTURE],
              env: { Y2_MCP_PID_PATH: projectPid },
            },
          },
        }),
      );
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot("y2-acp-project-mcp-optional-");
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot("y2-acp-project-mcp-reduce-");
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot("y2-acp-project-mcp-new-reduce-");
      const pidPath = join(root.root, "active-project-mcp.pid");
      writeFileSync(
        join(root.home, ".y2", "settings.json"),
        JSON.stringify({
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot("y2-acp-project-mcp-subagent-reduce-");
      ...
      const gateway = startDynamicFakeGateway((body) => {
        if (findOpenAiToolResult(body, selectId)) {
          return fakeGatewayToolCall(callId, MCP_TOOL_NAME, { text: "stall" });
        }
        if (findOpenAiToolResult(body, createId)) {
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot("y2-acp-paged-session-list-");
      const gateway = startFakeGateway([
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot("y2-acp-null-session-list-");
      try {
        client = await AcpClient.create({
          cwd: root.workspace,
          env: {
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot("y2-acp-relative-session-list-");
      try {
        client = await AcpClient.create({
          cwd: root.workspace,
          env: {
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot("y2-acp-legacy-session-list-");
      try {
